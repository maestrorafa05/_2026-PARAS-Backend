...
public static class RoomEndpoints
{
    ...
    public static IEndpointRouteBuilder MapRoomEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // endpoint untuk mendapatkan daftar semua ruangan
        ...
        // endpoint untuk membuat data ruangan baru
        group.MapPost("/", async (CreateRoomRequest req, ParasDbContext db) =>
        {
            ...
            var codeExists = await db.Rooms.AnyAsync(r => r.Code == req.Code);
            if (codeExists)
                return Results.BadRequest($"Room code '{req.Code}' sudah digunakan");
        ...
        // endpoint untuk mengupdate data ruangan berdasarkan id
        group.MapPut("/{id:guid}", async (Guid id, UpdateRoomRequest req, ParasDbContext db) =>
        {
            ...
            var codeExists = await db.Rooms.AnyAsync(r => r.Code == req.Code && r.Id != id);
            if (codeExists)
                return Results.BadRequest($"Room code '{req.Code}' sudah digunakan");
        ...
        // endpoint untuk menghapus data ruangan berdasarkan id
        group.MapDelete("/{id:guid}", async (Guid id, ParasDbContext db) =>
        {
            // cari ruangan berdasarkan id
            var room = await db.Rooms.FirstOrDefaultAsync(r => r.Id == id);
            if (room is null) return Results.NotFound();
            // tandai ruangan sebagai tidak aktif
            room.IsActive = false;
            room.UpdatedAt = DateTime.UtcNow;
            await db.SaveChangesAsync();
